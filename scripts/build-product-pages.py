...
<link rel="stylesheet" href="{prefix}styles.css"><link rel="stylesheet" href="{prefix}pages.css"><link rel="stylesheet" href="{prefix}navigation.css"><link rel="stylesheet" href="{prefix}updates-design.css"><script src="{prefix}navigation.js" defer></script><script src="{prefix}pages.js" defer></script></head>
...
# Reuse the project's attributed Lucide paths at build time, without a JS dependency.
ICON_PATHS = dict(re.findall(r'"([a-z0-9-]+)": `([^`]+)`', (ROOT / 'assets/icons.js').read_text()))
def glyph(name):
    return f'<svg class="release-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">{ICON_PATHS[name]}</svg>'

VISUALS = {
    'september-2026-platform-polish': ('pearl', 'layers', 'Everyday, refined.', 'A smoother place to work', [('panel-left', 'Interface', 'Thoughtful refinements'), ('workflow', 'Platform', 'Stability improvements')]),
    'august-2026-long-term-archiving': ('sand', 'history', 'Room for your history.', 'Today’s work. Long-term context.', [('layers', 'Daily operations', 'Responsive and focused'), ('folder', 'Historical information', 'Efficient cloud archiving')]),
    'august-2026-ai-cost-visibility': ('mint', 'chart-column', 'A clearer view of usage.', 'Understand. Review. Export.', [('chart-column', 'Model-level estimates', 'A breakdown of AI usage costs'), ('file-text', 'CSV reports', 'Bring estimates into your planning')]),
    'august-2026-faster-tower': ('lilac', 'tower-control', 'Ready when you are.', 'Pick up where you left off', [('tower-control', 'Tower', 'Loads when you need it'), ('panel-left', 'Your workspace', 'Your view stays in place')]),
    'august-2026-agent-execution-controls': ('sand', 'sliders-horizontal', 'Your rules. Every run.', 'Clear boundaries for automation', [('history', 'Timeout limits', 'Applied during execution'), ('settings-2', 'Evaluation criteria', 'Your configured rules, respected')]),
    'august-2026-search-and-workflow-control': ('mint', 'workflow', 'Stay in control.', 'Fewer interruptions. Clearer actions.', [('book-open', 'AI search', 'Recover from dropped connections'), ('workflow', 'Workflow controls', 'Stop an active run immediately')]),
    'july-2026-workflow-chat-widgets': ('lilac', 'message-square', 'Meet customers where they are.', 'Your workflow, on your website', [('message-square', 'Website chat', 'Powered by your workflows'), ('settings-2', 'Widget settings', 'Create, edit, enable, install')]),
    'july-2026-model-choice-and-usage': ('pearl', 'brain', 'More choice. Less friction.', 'The right model for the work', [('brain', 'Model selection', 'More options through OpenRouter'), ('chart-column', 'Usage summaries', 'A clearer view of active work')]),
    'july-2026-tower-and-team-context': ('lilac', 'tower-control', 'Keep your context close.', 'Your assistant, alongside your work', [('tower-control', 'Tower sidebar', 'A session that stays with you'), ('users', 'Shared knowledge', 'Organization and department context')]),
}

def visual(r, compact=False):
    tone, icon, headline, caption, items = VISUALS[r['slug']]
    rows = ''.join(f'<div class="visual-feature">{glyph(symbol)}<div><strong>{e(title)}</strong><span>{e(detail)}</span></div></div>' for symbol, title, detail in items)
    return f'<figure class="release-visual tone-{tone}{" is-compact" if compact else ""}"><div class="visual-heading"><span class="visual-mark">{glyph(icon)}</span><span>MACH 1 / {e(r["category"].upper())}</span></div><p class="visual-title">{e(headline)}</p><div class="visual-window"><div class="visual-window-bar"><span>{e(caption)}</span><span class="visual-window-dot" aria-hidden="true"></span></div>{rows}</div><figcaption>Feature illustration</figcaption></figure>'

def card(r):
    return f'''<article class="update-card">{visual(r, True)}<div class="update-card-copy"><p class="eyebrow">{e(r['category'])}</p><p class="period">{e(r['period'])}</p><h3><a href="{release_link(r)}">{e(r['title'])}</a></h3><p>{e(r['summary'])}</p><a class="textlink" href="{release_link(r)}">Explore the update <span aria-hidden="true">→</span></a></div></article>'''
...
<section class="page-cta"><div class="wrap"><p class="eyebrow">See it in context</p><h2>Meet your team’s next advantage.</h2><p>Connect your conversations. Choose Sales or Support. Find your next clear step.</p><p><a href="sample.html">Explore a sample workspace</a></p><div class="cta-row"><a class="btn light lg" href="preview.html">Get Started ↗</a><a class="btn outline lg" href="sales.html">Contact Sales</a></div></div></section>'''
...
rows = ''.join(f'''<article class="release-row" data-category="{e(r['category'])}" data-search="{e(search_text(r),quote=True)}">{visual(r, True)}<div class="release-card-copy"><div class="release-date"><span class="release-tag">{e(r['category'])}</span><p>{e(r['period'])}</p></div><h2><a href="{release_link(r)}">{e(r['title'])}</a></h2><p>{e(r['summary'])}</p><a class="textlink" href="{release_link(r)}">Explore the update <span aria-hidden="true">↗</span></a></div></article>''' for r in RELEASES)
updates=f'''<section class="updates-hero"><div class="wrap"><nav class="breadcrumbs" aria-label="Breadcrumb"><ol><li><a href="product.html">Product</a></li><li><span aria-current="page">Updates</span></li></ol></nav><div class="updates-intro"><div><p class="eyebrow">The evolution of Mach 1</p><h1>Good work.<br>Getting better.</h1></div><div><p class="lede">Thoughtful details. Meaningful new capabilities. Every update gives your team a little more room to do its best work.</p><p class="coverage">{coverage_label(RELEASES)} · {len(RELEASES)} release summaries</p></div></div></div></section><section class="release-index"><div class="wrap"><div class="release-tools" hidden><div class="release-filters" role="group" aria-label="Filter releases">{''.join(f'<button type="button" data-filter="{v}" aria-pressed="{str(v=="All").lower()}">{v}</button>' for v in ['All','Capabilities','Experience','Reliability'])}</div><label class="release-search">Search updates<input type="search" id="releaseSearch" placeholder="Tower, chat, costs…"></label></div><p class="result-count" role="status" hidden></p><div id="releaseList">{rows}</div><p id="noResults" hidden>No updates match your search. Try another term or choose All.</p></div></section><section class="page-cta"><div class="wrap"><p class="eyebrow">Made for your next move</p><h2>See the bigger picture.</h2><p>Explore the capabilities behind the updates.</p><a class="btn light lg" href="product.html">Meet Mach 1 →</a></div></section>'''
...
for i,r in enumerate(RELEASES):
    highlights=''.join(f'''<section class="release-detail-section" id="improvement-{n+1}"><span class="improvement-icon">{glyph(VISUALS[r["slug"]][4][n % 2][0])}</span><p class="eyebrow">Improvement {n+1:02}</p><h2>{e(h['title'])}</h2><p>{e(h['description'])}</p><div class="outcome"><strong>What this means for your team</strong><p>{e(h['benefit'])}</p></div></section>''' for n,h in enumerate(r['highlights']))
    def itemtext(v):
    ...
    article=f'''<article><header class="release-hero"><div class="wrap"><nav class="breadcrumbs" aria-label="Breadcrumb"><ol><li><a href="../product.html">Product</a></li><li><a href="../updates.html" data-updates-return>Updates</a></li><li><span aria-current="page">Release notes</span></li></ol></nav><div class="release-hero-copy"><div><p class="eyebrow">{e(r['category'])} / Release notes</p><h1>{e(r['title'])}</h1><p class="lede">{e(r['summary'])}</p><p class="period">{e(r['period'])} · {len(r["highlights"])} {"improvement" if len(r["highlights"]) == 1 else "improvements"}</p></div>{visual(r)}</div></div></header><div class="wrap release-layout"><aside class="release-aside"><p class="eyebrow">Why it matters</p><p>{e(r['benefit'])}</p><nav class="release-contents" aria-label="In this release"><p class="eyebrow">In this release</p>{"".join(f'<a href="#improvement-{n+1}"><span>{n+1:02}</span>{e(h["title"])}</a>' for n,h in enumerate(r["highlights"]))}</nav><a class="textlink" href="../product.html">Explore Mach 1 →</a></aside><div class="release-body">{highlights}{extras}<div class="release-end"><h2>Put the improvements in context.</h2><p>See how Mach 1 brings knowledge, tools, and workflows together for your team.</p><a class="btn solid" href="../product.html">Explore the product →</a></div><nav class="adjacent-releases" aria-label="Adjacent releases">{adjacent}</nav></div></div></article>'''
